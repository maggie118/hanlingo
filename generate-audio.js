// generate-audio.js
// 使用 Edge-TTS 生成所有短语音??
// 音色: zh-CN-XiaoxiaoNeural (最自然的中文女??
// 语?? -10% (更适合语言学习)
...
// ============================================================
const CONFIG = {
  voice: 'zh-CN-XiaoxiaoNeural',  // 最佳中文女??
  rate: '-10%',                    // 放慢10%，适合学习
  pitch: '+0Hz',                    // 音高不变 (edge-tts 7.x 要求 Hz 格式)
  outputDir: 'audio',
};
...
const PHRASES = [
  ...
  // ===== Emergency & Health (10) =====
  { id: 'emergency-01', chinese: '救命！请帮我??20?? },
  ...
  // ===== Phone & Internet (10) =====
  { id: 'phone-01', chinese: '我想买一??SIM 卡?? },
  ...
  { id: 'phone-09', chinese: '哪里有免??Wi-Fi?? },
  { id: 'phone-10', chinese: '我的手机型号??iPhone?? },
...
async function generateAllAudio() {
  ...
  for (let i = 0; i < PHRASES.length; i++) {
    ...
    if (fs.existsSync(outputPath)) {
      ...
      if (stats.size > 1000) {
        console.log(`?? ${progress} 跳过: ${phrase.id}.mp3 (已存?? ${Math.round(stats.size/1024)}KB)`);
        skipCount++;
        continue;
      } else {
        // 文件损坏，重新生??
        console.log(`?? ${progress} 文件损坏: ${phrase.id}.mp3 (${stats.size}bytes)，重新生??..`);
    ...
    for (let attempt = 1; attempt <= 3 && !phraseSuccess; attempt++) {
      try {
        ...
        if (attempt === 1) {
          ...
          console.log(`?? ${progress} 重试 ${attempt}/3: ${phrase.id}.mp3`);
        }
        ...
        if (attempt < 3) {
          console.log(`?? ${progress} ??{attempt}次失?? ${error.message.substring(0, 80)}，等待重??..`);
          await new Promise(resolve => setTimeout(resolve, 1000 * attempt));
        } else {
          console.error(`??${progress} 失败(3次重试后): ${phrase.id}.mp3`);
